...
public sealed class ClientScriptConsoleHandler
{
    ...
    private void SummonFromMenu(string defname)
    {
        ...
        if (!Definitions.CharDefHelper.TryApplyDefName(creature, defname.Trim(), resources, refresh: false, fireCreate: true))
        {
            creature.Delete();
    ...
    public IReadOnlyList<IScriptObj> QueryScriptObjects(string query, IScriptObj target, string args, ITriggerArgs? triggerArgs) =>
        ScriptObjectQueries.Query(_world, target, query, args, _commands?.Resources ?? DefinitionLoader.StaticResources);
