...
These exist in `TriggerTypes.cs` (and have name mappings) but have **no** `FireCharTrigger` / `FireItemTrigger` call site, so scripts hooking them will not run today:

**Character:** CallGuards, CharAttack, CharClick, CharClientTooltip, CharContextMenuRequest, CharContextMenuSelect, CharDClick, CharTradeAccepted, CombatAdd, CombatDelete, CombatEnd, EffectAdd, EnvironChange, ExpChange, ExpLevelChange, FameChange, KarmaChange, Eat, Follow, HitIgnore, HouseDesignCommit, HouseDesignExit, Jail, MurderDecay, MurderMark, NotoSend, NPCSeeNewPlayer, NPCSpecialAction, NPCRefuseItem, NPCSeeWantItem, NPCLostTeleport, PartyDisband, PersonalSpace, PetDesert, StatChange, ToolTip, SpellSelect, SpellBook, SkillChange, SkillMenu, SkillUseQuick, SkillWait, and the `User*` extended buttons (UserBugReport, UserExWalkLimit, UserGlobalChatButton, UserKRToolbar, UserMailBag, UserQuestArrowClick, UserSpecialMove, UserUltimaStoreButton, UserVirtue).

**Item:** SpellEffect, MemoryEquip, Redeed, RegionEnter, RegionLeave, ShipMove, ShipStop, ShipTurn, Smelt, Start, Stop, Level, Complete, AddRedCandle, AddWhiteCandle, DelRedCandle, DelWhiteCandle, Pickup_Self, Pickup_Stack, ResourceGather, ResourceTest (the last two fire via the resource path above, not as item triggers).
