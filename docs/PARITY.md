...
| Guild stone + party verb surfaces | Implemented | `SourceXVerbInventoryGuardrailTests` | Both tables are pinned now. SphereNet routes all 13 stone verbs upstream's own `CItemStone::r_Verb` handles and all 10 party verbs; the stone's other 17 table entries fall to `default: return false` upstream (leftovers of the stone menu) and are deliberately unrouted here, with `DeadTableEntries_AreStillDeadUpstream` recomputing that set from the reference source so the day upstream implements one, the guardrail asks for it here too. |
...
| `DEFMSG` runtime override | Implemented | `ParityWaveI1Tests`, `ServerMessagesTests`, `DefMsgRuntimeOverrideTests` | `DEFMSG.*` reads and `DEFMSG name=value` runtime override flow through `ServerMessages`. Runtime changes are deliberately NOT persisted, which is parity: upstream keeps default messages in `CServerConfig`, loaded from `sphere_msgs.scp`, and the world save never writes them (`CWorld.cpp` only reads a message to broadcast it). Saving them would give one key two sources of truth - the script pack an operator edits and an invisible saved value that silently wins. |
...
| Relay/game crypto matrix | Implemented | `EncryptionTests`, `GameStreamEncryptionTests` | Game-stream vectors per type: `ENC_BFISH` and `ENC_TFISH` round-trip at 16/255/256/257/1000 bytes (the 256-byte table refresh on both sides of its boundary), `ENC_BTFISH` round-trips through both layers, a payload arriving in five uneven pieces decrypts the same as one call for all three streams, a different seed is a different keystream, and the outgoing MD5 digest follows the seed. Measured while writing them: the BTFISH layers commute, because each is a XOR against a keystream that depends only on the seed and the byte count - the engine keeps upstream's order because it is upstream's, not because the result needs it. |
...
| `SAVESTATICS` | Implemented | `HandleServSaveStatics` → `WorldSaver.ExportStatics`, `StaticsExportTests` | Exports `ATTR_STATIC` world items to a text `.scp`, which is what upstream writes too: `CWorld::SaveStatics` (CWorld.cpp:1233) walks the sectors and writes a SCRIPT file through `OpenScriptBackup`, not `statics0.mul` — the "native static-map file" this row used to promise is not an upstream behaviour. Multis and items off the ground are skipped as upstream skips them, the file closes with `[EOF]`, the previous export is now RETIRED through the save backup rotation rather than overwritten, and the save is announced to every player first (`DEFMSG_SERVER_WORLDSTATICSAVE`). |
