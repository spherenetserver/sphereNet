...
public static partial class Program
{
    ...
    private static string B(bool? value) => value == true ? "1" : "0";
    ...
    private static string? ResolveServerProperty(string property)
    {
        ...
        return upper switch
        {
            ...
            // Source-X CServerConfig keys (r_WriteVal readback of the ELEM table).
            "ALLOWBUYSELLAGENT" => B(_config?.AllowBuySellAgent),
            "ALLOWNEWBTRANSFER" => B(_config?.AllowNewbTransfer),
            "AREAFLAGS" => (_config?.AreaFlags ?? 0).ToString(),
            "ARRIVEDEPARTMSG" => (_config?.ArriveDepartMsg ?? 0).ToString(),
            "AUTONEWBIEKEYS" => B(_config?.AutoNewbieKeys),
            "AUTOPRIVFLAGS" => (_config?.AutoPrivFlags ?? 0).ToString(),
            "AUTOSHIPKEYS" => B(_config?.AutoShipKeys),
            "CANSEESAMEPLEVEL" => (_config?.CanSeeSamePLevel ?? 0).ToString(),
            "CHARTAGS" => B(_config?.CharTags),
            "CHATSTATICCHANNELS" => _config?.ChatStaticChannels ?? "",
            "CONNECTINGMAXIP" => (_config?.ConnectingMaxIP ?? 0).ToString(),
            ...
            "ERALIMITLOOT" => (_config?.EraLimitLoot ?? 0).ToString(),
            "ERALIMITPROPS" => (_config?.EraLimitProps ?? 0).ToString(),
            "EXPERIENCEKOEFPVM" => (_config?.ExperienceKoefPVM ?? 0).ToString(),
            "EXPERIENCEKOEFPVP" => (_config?.ExperienceKoefPVP ?? 0).ToString(),
            "EXPERIENCEMODE" => (_config?.ExperienceMode ?? 0).ToString(),
            "EXPERIENCESYSTEM" => B(_config?.ExperienceSystem),
            "GUESTSMAX" => (_config?.GuestsMax ?? 0).ToString(),
            "LEVELMODE" => (_config?.LevelMode ?? 0).ToString(),
            "LEVELNEXTAT" => (_config?.LevelNextAt ?? 0).ToString(),
            "LEVELSYSTEM" => B(_config?.LevelSystem),
            "MAXCOMPLEXITY" => (_config?.MaxCharComplexity ?? 0).ToString(),
            "MAXCONNECTREQUESTSPERIP" => (_config?.MaxConnectRequestsPerIP ?? 0).ToString(),
            "MAXPINGS" => (_config?.MaxPings ?? 0).ToString(),
            "MAXSECTORCOMPLEXITY" => (_config?.MaxSectorComplexity ?? 0).ToString(),
            "MAXSHIPSGUILD" => (_config?.MaxShipsGuild ?? 0).ToString(),
            "MEDIUMCANHEARGHOSTS" => (_config?.MediumCanHearGhosts ?? 0).ToString(),
            "MOUNTHEIGHT" => B(_config?.MountHeight),
            "SPEECHOTHER" => _config?.SpeechOther ?? "",
            "STATSFLAGS" => (_config?.StatsFlags ?? 0).ToString(),
            "SUPPRESSCAPITALS" => B(_config?.SuppressCapitals),
            "TELNETLOG" => B(_config?.TelnetLog),
            "TIMEOUTINCOMPLETECONN" => (_config?.TimeoutIncompleteConnMs ?? 0).ToString(),
            "TIMERCALLUNIT" => B(_config?.TimerCallUnit),
            "TRADEWINDOWSNOOPING" => B(_config?.TradeWindowSnooping),
            "VENDORTRADETITLE" => B(_config?.VendorTradeTitle),
            "VERBOSEITEMBOUNCE" => B(_config?.VerboseItemBounce),
            "WOPCOLOR" => (_config?.WopColor ?? 0).ToString(),
            "WOPFONT" => (_config?.WopFont ?? 0).ToString(),
            "WOPPLAYER" => B(_config?.WopPlayer),
            "WOPSTAFF" => B(_config?.WopStaff),
            "WOPTALKMODE" => (_config?.WopTalkMode ?? 0).ToString(),
            "ZEROPOINT" => _config?.ZeroPoint ?? "",
