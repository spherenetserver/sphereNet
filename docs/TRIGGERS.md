...
- **P1 (4)** — moderate: ExpChange, ExpLevelChange, SkillUseQuick, EnvironChange. *(Wired since: `Eat`, `SkillMenu`, `SkillWait` (IsTrigUsed-gated), `Follow`, `PartyDisband`, `SpellSelect`, `SpellBook`, `PersonalSpace` (movement shove), `EffectAdd` (spell effect applied, IsTrigUsed-gated), `PetDesert` (pet loyalty hits zero in `TickPetOwnershipTimers`; the decay/feed/go-wild loop already existed — only the trigger was missing), `Jail` (the GM JAIL command; jail teleport/freeze/timed-release already existed — only the trigger was missing), `CallGuards` (the "guards" keyword handler in `Program.NpcServices` — the guard-summon system already existed, fires per reported criminal with `<argo>` = the hostile, RETURN 1 cancels). Deferred: `SkillUseQuick` — `UseQuick` is atomic, pre-roll cancel needs a check/gain split; `ExpChange`/`ExpLevelChange` — no runtime experience/level system (`Exp`/`Level` are persistence-only fields); `EnvironChange` — needs per-character light/weather state to fire only on an actual change.)*
